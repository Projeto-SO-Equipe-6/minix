...
void enqueue(struct proc *rp) {
    ...
    if ((F_Fifo + 1) % MAX_FIFO == I_Fifo) {
        panic("Fila cheia!");
    }
        FIFO[F_Fifo] = rp;
        F_Fifo = (F_Fifo + 1) % MAX_FIFO;
...
void dequeue(struct proc *rp) {
    ...
    while (i != F_Fifo) {
        if (FIFO[i] == rp) {
            j = i;
            int fim_real = (F_Fifo - 1 + MAX_FIFO) %
MAX_FIFO;
            while(j != fim_real) {
                FIFO[j] = FIFO[(j + 1) % MAX_FIFO];
                j = (j+1) % MAX_FIFO;
            }
            F_Fifo = (F_Fifo - 1 + MAX_FIFO) %
MAX_FIFO;
