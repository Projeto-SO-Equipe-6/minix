...
void dequeue(struct proc *rp) {
    ...
    while (i != fim_fila_sjf) {
        if (fila_sjf[i] == rp) {
            j = i;
            int fim_real = (fim_fila_sjf - 1 + TAM_MAX_FILA) % TAM_MAX_FILA;
            while(j != fim_real) {
                fila_sjf[j] = fila_sjf[(j + 1) % TAM_MAX_FILA];
                j = (j+1) % TAM_MAX_FILA;
            }
            fim_fila_sjf = (fim_fila_sjf - 1 + TAM_MAX_FILA) % TAM_MAX_FILA;
            break;
        }
        i = (i+1) % TAM_MAX_FILA;
    }
